...
def lambda_handler(event, context):
    # 実行日前月のAWS利用コストを取得
    ce_input_day_end = datetime.now().replace(day=1) - timedelta(days=1)
    ce_input_day_start = ce_input_day_end.replace(day=1)
    ce_client = boto3.client('ce', verify=False)
    ...
    for item in monthly_costs_per_group:
        amount = float(item['Metrics']['BlendedCost']['Amount'])
        if item['Keys'][0] == 'Env$':
    ...
    empty_tag_occupancy = 100 * empty_tag_amount / \
        (empty_tag_amount + allocated_tags_amount)
    messages.append("")
    messages.append(
        f"タグの付いていないリソースのコスト占有率が **{empty_tag_occupancy:.2f}%** 存在します！")
    messages.append("")
    ...
    r = requests.post(os.environ['MATTERMOST_WEBHOOK_URL'],
                      headers=headers, data=json.dumps(payload))
    if r.status_code != 200:
        raise Exception(
            f"Post failed with status code: {r.status_code}, message: {r.text}")
    return
